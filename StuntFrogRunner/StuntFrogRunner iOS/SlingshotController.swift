...
class SlingshotController {
    ...
    init(scene: SKScene) {
        ...
        if let gs = scene as? GameScene {
            self.frogController = gs.frogController
        }
    }
    ...
    func handleTouchBegan(at location: CGPoint, frogScreenPosition: CGPoint) {
        ...
        if distance < 80 {
            ...
            frogController?.pauseSuperJumpCountdown = true
    ...
    func handleTouchEnded(at location: CGPoint, frogScreenPosition: CGPoint, frogWorldPosition: CGPoint, worldOffset: CGFloat, superJumpActive: Bool) -> CGPoint? {
        ...
        if pullDistance < GameConfig.minPullDistance {
            frogController?.pauseSuperJumpCountdown = false
    ...
    func drawSlingshot(frogScreenPosition: CGPoint, frogWorldPosition: CGPoint, superJumpActive: Bool, lilyPads: [LilyPad], worldNode: SKNode, scene: SKScene, worldOffset: CGFloat) {
        ...
        linePath.move(to: frogScreenPosition)
        ...
        // Calculate and show target position
        let dx = pullCurrentPos.x - frogScreenPosition.x
        let dy = pullCurrentPos.y - frogScreenPosition.y
        ...
        let targetScreenX = frogScreenPosition.x + targetOffsetX
        let targetScreenY = frogScreenPosition.y + targetOffsetY
    ...
    func cancelCurrentAiming() {
        ...
        // Clear any visuals
        clearVisuals()
        // Ensure countdown resumes if it was paused
        frogController?.pauseSuperJumpCountdown = false
    }
}
