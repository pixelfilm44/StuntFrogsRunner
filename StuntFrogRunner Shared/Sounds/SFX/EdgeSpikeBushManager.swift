...
class EdgeSpikeBushManager {
    ...
    func checkCollisions(frogPosition: CGPoint, frogRadius: CGFloat = 25) -> Bool {
        let allEdgeBushes = leftEdgeSpikeBushes + rightEdgeSpikeBushes
        
        for bush in allEdgeBushes {
            let dx = frogPosition.x - bush.position.x
            let dy = frogPosition.y - bush.position.y
            let distance = sqrt(dx * dx + dy * dy)
            
            if distance < frogRadius + (GameConfig.edgeSpikeBushSize / 2) {
